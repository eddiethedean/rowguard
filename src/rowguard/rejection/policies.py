...
class RaisePolicy:
    def handle(self, rejected: RejectedRow) -> RejectionDecision:
        ...
        if rejected.adaptation_error is not None:
            ...
            if isinstance(cause, RowAdaptationError):
                error: BaseException = RowAdaptationError(
                    str(cause),
                    model=rejected.model,
                    row_index=rejected.index,
                )
        ...
        raise ResultAssemblyError("RaisePolicy requires a validation or adaptation error")
